...
public class DutchNationalFlag {
    ...
    public static void dutchFlagPartition(int pivotIndex, List<Color> A) {
        ...
        for (int i = 0; i < A.size(); i++) {
            if (A.get(i).ordinal() < pivotColor.ordinal()) Collections.swap(A, i, less++);
        }
        int greaterEqual = A.size()-1;
        for (int i = A.size()-1; i >= 0 && A.get(i).ordinal() >= pivotColor.ordinal(); i--) {
            if (A.get(i).ordinal() > pivotColor.ordinal()) Collections.swap(A,i, greaterEqual--);
        }
    }
