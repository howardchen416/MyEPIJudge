...
public class TreeRightSibling {
  ...
  public static void constructRightSibling(BinaryTreeNode<Integer> tree) {
    ...
    BinaryTreeNode<Integer> iL = tree;
    // go left
    while (iL.left!=null) {
      iL = constructRightSiblingAtLowerLevel(iL);
    }
    return;
  }

  public static BinaryTreeNode<Integer> constructRightSiblingAtLowerLevel(BinaryTreeNode<Integer> iL) {

    BinaryTreeNode<Integer> iter = iL;

    while (iter!=null) {
      // left child
      if (iter.left!=null) iter.left.next = iter.right;
      // right child
