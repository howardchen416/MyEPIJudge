...
public class Hanoi {
  ...
  public static List<List<Integer>> computeTowerHanoi(int numRings) {
    // special condition(s)
    if (numRings<1) return Collections.emptyList();
    ...
    for (int i = 0; i < numRings; i++) {
      pegs.get(0).addFirst(numRings-1-i);
    }

    steps(pegs, numRings, 0, 1,2);

    //
    List<List<Integer>> r = new ArrayList<>(NUM_PEGS);
    for (int i =0; i < pegs.size(); i++) {
      //pegs.set(i, (List) pegs.get(i)));
    }
  ...
  public static void steps(List<Deque<Integer>> pegs, int numRings, int from, int to, int use) {
    if (numRings==0) return;
    steps(pegs, numRings-1, from, use, to);
    pegs.get(to).addFirst(pegs.get(from).removeFirst());
    steps(pegs, numRings-1, use, to, from);
    return;
  }
