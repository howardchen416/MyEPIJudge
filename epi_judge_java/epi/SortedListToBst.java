...
public class SortedListToBst {
  ...
  public static DoublyListNode<Integer> buildBSTFromSortedList(DoublyListNode<Integer> L, int length) {
    head = L;
    DoublyListNode<Integer> result = buildBSTFromSortedListHelper(0, length);
    return result;
  }

  private static DoublyListNode<Integer>
          buildBSTFromSortedListHelper(int start, int end) {
    if (start >= end)
      return null;
    ...
    DoublyListNode<Integer> curr = new DoublyListNode<>(head.data, left, null);
    head = head.next;
    curr.next = buildBSTFromSortedListHelper(mid+1, end);
    return curr;
  }
